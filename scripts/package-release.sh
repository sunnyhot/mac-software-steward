...
if [ -f "$ROOT_DIR/release/RELEASE_NOTES-v$VERSION.md" ]; then
...
else
  {
    ...
    echo ""
    echo "### 维护体验与诊断"
    echo "- 普通 App 诊断展示更新源异常、无法确认版本、识别器、置信度、安装/可用版本、Feed URL 和处理建议"
    echo "- 普通 App 更新能力识别补充 Chrome Keystone、Adobe、JetBrains Toolbox、Microsoft AutoUpdate 的 Info.plist 元数据证据"
    echo "- 自动化策略页直接提供自动化、风险和恢复控制"
    echo "- 任务日志统一展示巡检、升级、待办处理历史，支持分类、状态和搜索筛选"
    echo "- 待处理事项集中显示在维护总览，来源异常在可升级页直接提供恢复操作"
    echo ""
    echo "### 安全与发布可靠性"
    ...
    echo "- 修复维护计划并行数量只作用于 job、包升级仍串行执行的问题"
    echo "- 维护计划现在会在 brew update 后按设置的并行数量同时执行多个包升级"
